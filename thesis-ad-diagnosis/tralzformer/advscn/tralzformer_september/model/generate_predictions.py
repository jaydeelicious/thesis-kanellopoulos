...
basedir="/home/kanellopoulos/thesis/thesis-ad-diagnosis/tralzformer/advscn/tralzformer_september/"

fname = 'Exp_07_09_DiagnosisPrediction_Cluster1_7V_ADvsCN_fold_1_Preds'
save_path = '/home/kanellopoulos/thesis/thesis-ad-diagnosis/tralzformer/advscn/tralzformer_september/model_predictions/'
dat_file = '/home/kanellopoulos/thesis/thesis-ad-diagnosis/tralzformer/advscn/tralzformer_september/data/adni/FullCohort_Test_Cluster1.csv'
cnf_file = f'{basedir}/data/toml/diagnosis_prediction_7_visits.toml'
ckpt_path = '/home/kanellopoulos/thesis/thesis-ad-diagnosis/tralzformer/advscn/tralzformer_september/ckpt/Exp_07_09_DiagnosisPrediction_7V_ADvsCN_fold_1.pt'
...
def generate_performance_report(dat_tst, y_pred, scores_proba):
    ...
    # # AD vs CN
    # #y_true_arr.squeeze(-1)
    # binary_y_true_test = (y_true_arr == 2).astype(np.int32)

    # adcn_mask = ((y_true_arr == 0) | (y_true_arr == 2))
    # y_mask_adcn = (mask_arr.bool() & adcn_mask).to(mask_arr.dtype)

    # y_prob_test = torch.softmax(scores_arr, dim=-1)[..., 2]
    # thr = 0.5
    # binary_y_pred_tst = (y_prob_test > thr).int() 
    ...
    # AD vs CN mask
    adcn_mask   = ( (y_true == 0) | (y_true == 2) )   # (N, T)
    y_mask_adcn = (mask & adcn_mask).to(mask.dtype)   # (N, T)
    ...
    if scores.ndim == 3 and scores.size(-1) == 3:
        # logits -> P(AD)
        y_prob_test = torch.softmax(scores, dim=-1)[..., 2]   # (N, T)
    else:
    ...
    # Metrics (expects (N, T) everywhere)
    met_pred = misc.get_metrics_multitask(
        binary_y_true_test.cpu().numpy(),
        binary_y_pred_tst.cpu().numpy(),
        y_prob_test.cpu().numpy(),
        y_mask_adcn.cpu().numpy()
    )
