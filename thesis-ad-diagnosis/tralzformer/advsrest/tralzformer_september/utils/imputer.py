...
class FrequencyImputer(Imputer):
    ...
    def __call__(self, 
        ...
        #print(f"🔍 Inside FrequencyImputer.__call__()")
        #print(f"✅ Type of self: {type(self)}")  # Should be FrequencyImputer
        #print(f"✅ Type of smp: {type(smp)}")  # Should be dict
        #print(f"✅ smp keys: {list(smp.keys())}")  # Check structure

        if smp is None:
            raise RuntimeError("❌ ERROR: `smp` is None before processing!")
            ...
            if smp[k] is not None:
                ...
                if info['type'] == 'categorical':
                    ...
                    if info['type'] == 'numerical':
                        if not self.pool[k]:
                            raise RuntimeError(f"❌ ERROR: Pool for key `{k}` is empty!")
                        rnd_idx = np.random.randint(0, len(self.pool[k]))
                        new[k] = np.array(self.pool[k][rnd_idx])
                        # print(type(new[k]))
                    else:
                        #ic(info['shape'])
                        raise ValueError(f"Unknown type: {info['type']} for key {k}")
                        #raise ValueError
        return new