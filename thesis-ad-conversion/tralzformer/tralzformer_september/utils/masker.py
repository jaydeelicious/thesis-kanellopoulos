...
class DropoutMasker(Masker):
    ...
    def __call__(self,
        ...
        if self.dropout_strategy == 'permutation':
            ...
            mask = {k: False for k in self.modalities}
            for k in src_keys:
                mask[k] = False
            missing_mask = {
                k: (smp[k] is None or np.all(smp[k] == 0.0))
                for k in self.modalities
            }

            mask = {k: (mask[k] or missing_mask[k]) for k in self.modalities}
            return mask
...
class LabelMasker():
    ...
    def __call__(self,
        ...
        ''' ... '''
        # get missing mask
        #label_mask = {k: True if smp[k] is not None else False for k in self.modalities}
        label_mask = {
            k: False if (smp[k] is None or smp[k] == -1) else True
            for k in self.modalities
        }
        # print(label_mask)
