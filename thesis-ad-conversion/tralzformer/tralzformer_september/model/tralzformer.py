...
class TralzformerModel(BaseEstimator):
    ...
    def fit(self, x_trn, x_vld, y_trn, y_vld, train_ids, val_ids) -> Self:
        ...
        if self.wandb_ == 1:
            ...
            wandb.init(
                ...
                project="Tralzformer_Final_Results",
                group="Exp_08_09_ConversionPrediction_7V",
                name=run_name,
        ...
        for k in self.tgt_labels:
            ...
            self.loss_fn[k] = focal_loss.SigmoidFocalLoss(
                alpha = alpha,
                gamma = self.gamma,
                reduction = 'none'
            )
    ...
    def train_one_epoch(self, ldr_trn, epoch):
                ...
                if self.ranking_loss:
                    if epoch < 10:
                        ...
                        for i, k in enumerate(self.tgt_labels):
                            for ii, kk in enumerate(self.tgt_labels):
                                if ii>i:
        ...
        y_true_trn = torch.cat(y_true_trn).squeeze(-1)
        y_mask_trn = torch.cat(y_mask_trn)
        #y_pred_trn = (scores_trn > 0).to(torch.int).squeeze(-1)
        y_prob_trn = torch.sigmoid(scores_trn).squeeze(-1)

        binary_y_true_trn = (y_true_trn == 2).int()

        mcicnn_mask = ((y_true_trn == 1) | (y_true_trn == 2))
        y_mask_mcicn = (y_mask_trn.bool() & mcicnn_mask).to(y_mask_trn.dtype)

        # new y_pred_trn
        # minimal change: pick threshold by matching prevalence on the masked set
        _mask = y_mask_mcicn.bool()
        if _mask.any() and (binary_y_true_trn[_mask].sum() > 0):
            pos_rate = binary_y_true_trn[_mask].float().mean()
            thr = torch.quantile(y_prob_trn[_mask], 1 - pos_rate)  # top-k threshold
        else:
            thr = torch.tensor(1.0, device=scores_trn.device)  # no positives -> predict none

        y_pred_trn = (y_prob_trn >= thr).to(torch.int)

        met_trn = misc.get_metrics_multitask(
            binary_y_true_trn.numpy(),
            y_pred_trn.numpy(),
            y_prob_trn.numpy(),
            y_mask_mcicn.numpy()
        )
    ...
    def validate_one_epoch(self, vld_loader, epoch):
        ...
        y_true_vld = torch.cat(y_true_vld).squeeze(-1)
        y_mask_vld = torch.cat(y_mask_vld)
        #y_pred_vld = (scores_vld > 0).to(torch.int).squeeze(-1)
        y_prob_vld = torch.sigmoid(scores_vld).squeeze(-1)

        binary_y_true_vld = (y_true_vld == 2).int()

        mcicnn_mask = ((y_true_vld == 1) | (y_true_vld == 2))
        y_mask_mcicn = (y_mask_vld.bool() & mcicnn_mask).to(y_mask_vld.dtype)

        # new y_pred_vld
        # minimal change: pick threshold by matching prevalence on the masked set
        _mask = y_mask_mcicn.bool()
        if _mask.any() and (binary_y_true_vld[_mask].sum() > 0):
            pos_rate = binary_y_true_vld[_mask].float().mean()
            thr = torch.quantile(y_prob_vld[_mask], 1 - pos_rate)  # top-k threshold
        else:
            thr = torch.tensor(1.0, device=scores_vld.device)  # no positives -> predict none

        y_pred_vld = (y_prob_vld >= thr).to(torch.int)
    ...
    def predict_proba(self,
        ...
        for smp in logits:  # for each sample
            ...
            for t, vals in smp.items():  # for each time step
                # make sure z is a 1-elem torch tensor (CPU, no grad)
                if isinstance(vals, torch.Tensor):
                    z = vals.detach()
                else:
                    z = torch.as_tensor(vals, dtype=torch.float32)
                z = z.reshape(-1)[0].float() / max(temperature, 1e-8)

                p1 = torch.sigmoid(z).item()  # scalar P(conversion at t)
                smp_proba[t] = float(p1)
    ...
    def predict(self,
        ...
        if fpr is None or tpr is None or thresholds is None:
            ...
            # Binary conversion (MCI->AD) with scalar probs per time step
            threshold = 0.25  # or your dev-selected t*
            ...
            for smp in proba:  # over samples
                ...
                for t, p in smp.items():  # p is already a scalar prob
                    p = float(p)  # ensure plain float
                    smp_pred[t] = int(p >= threshold)
                    smp_prob[t] = p
            ...
            for smp_logit in logits:
                ...
                for t in smp_prob.keys():
                    smp_logit_pos[t] = float(smp_logit[t][0])
    ...
    def __init_net(self):
        ...
        if self.data_parallel and torch.cuda.device_count() > 1:
             ...
             self.net_ = torch.nn.DataParallel(self.net_, device_ids=[self.cuda_devices[0]])
