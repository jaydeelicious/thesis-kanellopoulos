...
class Transformer(torch.nn.Module):
    def __init__(
            ...
            self.time_step_emb = torch.nn.Embedding(max_time_steps, d_model)

            self.w_age = torch.nn.Parameter(torch.tensor(0.5))

            # Gap-aware time embeddings
            # Δt: months since previous visit
            self.dt_proj = torch.nn.Sequential(
            ...
            # One prediction head shared among time steps
            self.target_preds = torch.nn.ModuleDict()
            shared_conv_head = torch.nn.Linear(d_model, 1)
            for k, info in tgt_labels.items():
                if info['type'] == 'categorical' and info['num_categories'] == 2:
                    self.target_preds[k] = shared_conv_head
                else:
    ...
    def forward(self,
            ...
            times = sorted(out_emb.keys())
            sample = next(iter(out_emb[times[0]].values()))
            device, dtype = sample.device, sample.dtype

            # Δt as [B, T, 1] (months)
            delta_t = self.assemble_delta_t_from_x(x, times, device=device, dtype=dtype)

            out_trf = self.forward_trf(out_emb, mask, delta_t)
    ...
    def forward_trf(self, out_emb, mask, delta_t):
        ...
        combined_emb = combined_emb + self.w_age * age_stack  

        delta_t = torch.log1p(delta_t)
        dt_emb = self.dt_proj(delta_t)
        combined_emb = combined_emb + dt_emb
    ...
    def assemble_delta_t_from_x(self, x, times, *, device, dtype, cap_months=60.0):
        """Return [B, T, 1] Δt in MONTHS from per-time keys in x."""
        vals = []
        # find a reference tensor to infer B/device/dtype for missing timesteps
        ref = None
        for t in times:
            if f"time{t}_delta_t_months" in x or f"time{t}_delta_t" in x:
                ref = x.get(f"time{t}_delta_t_months", x.get(f"time{t}_delta_t"))
                break

        for t in times:
            km = f"time{t}_delta_t_months"
            ky = f"time{t}_delta_t"
            if km in x:
                v = x[km]                        # [B, 1] months
            elif ky in x:
                v = x[ky] * 12.0                 # years -> months
            else:
                if ref is None:
                    raise ValueError("No delta_t keys found in x to infer batch size.")
                v = torch.zeros_like(ref)        # [B, 1]
            vals.append(v)

        dt = torch.stack(vals, dim=1).to(device=device, dtype=dtype)  # [B, T, 1]
        dt = dt.clamp_min(0.0)
        if cap_months is not None:
            dt = dt.clamp_max(cap_months)
        dt[:, 0, :] = 0.0                        # baseline = 0
        return dt
